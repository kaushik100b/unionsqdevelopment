...
!(function (e, t) {
		...
		if (o) {
			...
			(p = 'scroll' + n.d2),
				X(
					...
					(_ && 20 < S) ||
						(!_ &&
							(u ? Math.max(Ge[p], qe[p]) : o.parentNode[p]) <= S + 1)
				),
				u &&
					((l = Mt(a)),
				...
				for (
					...
					(r = e[o + 1]),
						...
						r
							? (n[t] = r)
							: n[t] &&
							  n.removeProperty(t.replace(ne, '-$1').toLowerCase());
			}
		},
		Bt = {left: 0, top: 0},
					...
					if (
						...
						ie &&
							(!1 === oe ||
								oe === _t ||
								(oe =
									!(!oe && 'flex' === db(ie.parentNode).display) &&
									St),
							(Se.pin = ie),
							...
							if ((!rt && Se.enabled) || t)
								if (ie && e && pt) nb(ScrollTrigger, 'scrollEnd', Jb);
									...
									f &&
										...
										ie &&
											oe &&
											(L._pinOffset = Math.round(Se.progress * N)),
										te && te(Se);
								}
						}),
		...
		return (
			...
			(e.onChange = function (e, t, r, n, i) {
				G !== a && rp(),
					t &&
						f &&
						v(S(n[2] === t ? l + (e.startX - e.x) : v() + t - n[1])),
	...
	(oe.sort = function (e) {
		...
		(oe.normalizeScroll = function (e) {
			...
			return (
				h && h.target === t.target && h.kill(), Ba(t.target) && (h = t), t
			);
		}),
		(oe.core = {
			_getVelocityProp: K,
